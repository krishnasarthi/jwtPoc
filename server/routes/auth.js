            ...
            request.get({ url: peopleApiUrl, headers: headers, json: true }, function (err, response, profile) {
                ...
                connectionPool.getConnection(function (err, connection) {
                    ...
                    if (req.header('Authorization')) {
                        connection.query('select user_id,email,password from user where google_id = ?', profile.sub, function (err, existingUser) {
                            if (existingUser) {
                            ...
                            connection.query('select user_id,email,password from user where user_id = ?', payload.sub, function (err, user) {
                                if (!user) {
                                    return res.status(400).send({ message: 'User not found' });
                                }

                                connection.query('update user SET google_id = ?', profile.sub, function (err, result) {
                                    if (err) {
                        ...
                        connection.query('select user_id,email,password from user where google_id = ?', profile.sub, function (err, existingUser) {
                            ...
                            if (existingUser && existingUser.length>0) {
                                return res.send({ token: security.createToken(existingUser.user_id, req) });
                            }
