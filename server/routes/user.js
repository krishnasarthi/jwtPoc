...
module.exports = function (app) {
    ...
    app.post('/api/login', function (req, res) {
        ...
        connectionPool.getConnection(function (err, connection) {
            ...
            connection.query('select user_id,email,password from user where email = ?', user.email, function (err, row) {
                connection.release();
                if (err){
                    res.status(-1).send({ message: err });
                    return;
                ...
                if (!row || row.length == 0) {
                    res.status(-1).send({ message: 'No user with email : ' + user.email });
                    return;
                }

                var password = row[0].password;

                security.comparePassword(password, user.password, function (err, isMatch) {
                    if (err){
                        res.status(-1).send({ message: err });
                        return;
                    }
